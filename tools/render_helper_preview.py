"""Export the real four helper animations as a self-contained browser preview."""
import base64
import io
import json
...
def main():
    ...
    items = []
    colors = ('#df916d',) * len(artwork.HELPER_FIGURES)
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Little Smith · four approved poses</title>
<style>
...
.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:18px;margin-top:28px}.card{background:#201c17;border:1px solid #39322a;border-radius:14px;padding:22px;cursor:pointer}
...
</style><main><header><div><div class="eyebrow">SMITH AGENTS · LITTLE SMITH</div><h1>Four approved helper poses.</h1><p>Click a figure to replay. Each action settles into a quiet idle.</p></div>
